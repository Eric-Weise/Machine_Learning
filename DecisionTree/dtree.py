...
class dtree:
	...
	def info_gain(self,data,classData,featureIndex):
		...
		gain = 0
		data = np.array(data)
		feature = data[:, featureIndex]
		fEntropy = 0
		for i in self.featureValues[self.featureNames[featureIndex]]:
			list = []
			for x in range(len(classData)):
				if feature[x] == i:
					list.append(classData[x])
			fEntropy += self.entropy(list) * len(list)
		fEntropy = fEntropy / len(classData)
		gain = self.entropy(classData) - fEntropy
